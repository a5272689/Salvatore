...
function infohandle(handle,infoid) {
    ...
    var sn=$('#j-sn').val();
    var corenum=Number($('#j-corenum').val());
    ...
    if (!model) {$('.form_warning').text('CPU型号不能为空！');return}
    if(!corenum){$('.form_warning').text('CPU核心数不能为空,且必须为整数！');return}
    if(!nowspeed){$('.form_warning').text('CPU主频不能为空,且必须为数字！');return}
    var senddata={'model':model,'corenum':corenum,'nowspeed':nowspeed,'manufactor':manufactor};
    ...
    $.ajax({
    url:'/cpuhandleAPI/?method='+handle,
    contentType:'application/json;charset=utf-8',
    ...
    success:function (res) {
        if (res['result']){
            ...
            if (handle=='add'){$('.form_warning').text('添加CPU失败！')}
            else {$('.form_warning').text('修改CPU失败！')}
        }
    }});
