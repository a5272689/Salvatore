...
$('#info_del').click(function () {
    ...
    $.ajax({
        url:'/databasehandleAPI/?method=del',
...
function infohandle(handle,infoid) {
    ...
    if (!name) {$('.form_warning').text('数据库名称不能为空！');return}
    ...
    $.ajax({
    url:'/databasehandleAPI/?method='+handle,
    contentType:'application/json;charset=utf-8',
    ...
    success:function (res) {
        if (res['result']){
            ...
            if (handle=='add'){$('.form_warning').text('添加数据库失败！')}
            else {$('.form_warning').text('修改数据库失败！')}
        }
    }});
