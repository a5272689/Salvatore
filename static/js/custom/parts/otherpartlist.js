...
$('#info_del').click(function () {
    ...
    $.ajax({
        url:'/otherparthandleAPI/?method=del',
...
function infohandle(handle,infoid) {
    ...
    if (!model) {$('.form_warning').text('其它部件型号不能为空！');return}
    ...
    $.ajax({
    url:'/otherparthandleAPI/?method='+handle,
    contentType:'application/json;charset=utf-8',
    ...
    success:function (res) {
        if (res['result']){
            ...
            if (handle=='add'){$('.form_warning').text('添加其它部件失败！')}
            else {$('.form_warning').text('修改其它部件失败！')}
        }
    }});
