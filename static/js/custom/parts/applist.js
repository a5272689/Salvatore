...
function infohandle(handle,infoid) {
    ...
    $.ajax({
    url:'/apphandleAPI/?method='+handle,
    contentType:'application/json;charset=utf-8',
    ...
    success:function (res) {
        if (res['result']){
            ...
            if (handle=='add'){$('.form_warning').text('添加应用失败！')}
            else {$('.form_warning').text('修改应用失败！')}
        }
    }});
...
function changeinfo(index) {
    ...
    $('#j-language').val(rowdata['language']);
